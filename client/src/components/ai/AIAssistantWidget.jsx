    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap bg-slate-100 p-1.5 rounded-2xl gap-1 text-xs font-semibold">
          {[
            { id: 'triage', label: 'Clinical Triage', Icon: Sparkles },
            { id: 'cost', label: 'Cost & Stay Estimator', Icon: Calculator },
            { id: 'report', label: 'Report Explainer', Icon: FileSearch },
            { id: 'itinerary', label: 'Medical Itinerary', Icon: Plane },
            { id: 'chat', label: 'AI Q&A Chat', Icon: MessageSquare },
          ].map(({ id, label, Icon }) => (
            <button
              key={id}
        ...
        {activeTab !== 'chat' && (
          <div className="bg-gradient-to-r from-navy-950 via-slate-900 to-navy-950 text-white rounded-2xl p-5 shadow-lg border border-teal-500/30">
            <div className="flex items-center gap-2 text-teal-300 text-xs font-bold uppercase tracking-wider mb-1">
              ...
              {activeTab === 'triage' && 'Intelligent Clinical Triage'}
              {activeTab === 'cost' && 'International Price Benchmarking'}
              {activeTab === 'itinerary' && 'Personalized Medical Travel Planner'}
              {activeTab === 'report' && 'Diagnostic Findings Interpreter'}
            </div>
            <h4 className="text-lg sm:text-xl font-display font-bold">
              {activeTab === 'triage' && 'Describe your medical condition or desired surgical procedure'}
              {activeTab === 'cost' && 'Compare India healthcare costs vs USA, UK, Canada & UAE'}
              {activeTab === 'itinerary' && 'Day-by-Day Treatment, Hospital Stay & Tourism Timeline'}
              {activeTab === 'report' && 'Paste diagnostic scan findings or pathology summary'}
            </h4>
            <p className="text-slate-300 text-xs mt-1 leading-relaxed">
              {activeTab === 'triage' && 'Our clinical matching engine maps symptoms to JCI/NABH accredited centers, verifies top specialist surgeons, and calculates estimated recovery timelines.'}
              {activeTab === 'cost' && 'Transparent international price comparisons showing ~70-90% cost savings for high-end surgeries in India without compromising clinical excellence.'}
              {activeTab === 'itinerary' && 'Customized timeline spanning pre-op consultation, surgical admission, recuperation hotel stay, local wellness, and Fit-to-Fly certification.'}
              {activeTab === 'report' && 'Our AI translates complex radiology (MRI, CT, X-Ray) and laboratory terminology into clear explanations with direct specialist department recommendations.'}
            </p>
          </div>
        )}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <div className="p-3 rounded-xl bg-teal-50 border border-teal-200">
                  ...
                  <div className="text-base font-extrabold text-teal-700 mt-1">{formatPrice(result.estimatedCostUSD)}</div>
                  <div className="text-[10px] text-teal-600 font-semibold mt-0.5">★ Best Value & Zero Wait Time</div>
                </div>
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                  <div className="text-[10px] font-bold text-slate-500">United States (Average)</div>
                  <div className="text-base font-bold text-slate-800 mt-1">{formatPrice(result.costUSAUSD)}</div>
                ...
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                  ...
                  <div className="text-base font-bold text-slate-800 mt-1">{formatPrice(result.costUKUSD)}</div>
                ...
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                  ...
                  <div className="text-base font-bold text-slate-800 mt-1">{formatPrice(Math.round(result.estimatedCostUSD * 2.8))}</div>
            ...
            {result.recommendedDoctors?.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-500">Direct Pre-Travel Video Consultations Available</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {result.recommendedHospitals.map((hosp) => (
                    ...
                    >
                      <div className="font-bold text-slate-900 text-xs leading-snug">{hosp.name}</div>
                      <div className="text-[11px] text-slate-500">{hosp.city}, {hosp.state}</div>
